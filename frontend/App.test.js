...
describe("App", () => {
  ...
  test("renders error message when request fails", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Wystąpił chwilowy błąd./i)).toBeInTheDocument());
...
const mockRecipes = [
    ...
    await waitFor(() => expect(screen.getByText(/Recipe 1/i)).toBeInTheDocument());
    await waitFor(() => expect(screen.getByText(/Recipe 2/i)).toBeInTheDocument());
    await waitFor(() => expect(screen.getByText(/Recipe 3/i)).toBeInTheDocument());
